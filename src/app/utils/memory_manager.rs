...
mod tests {
    ...
    fn create_test_config() -> MemoryManagerConfig {
        MemoryManagerConfig {
            l1_cache_max_entries: 100,
            l1_cache_ttl_seconds: 10,
            object_pool_initial_size: 10,
            object_pool_max_size: 50,
            memory_pool_block_size: 1024,
            memory_pool_max_blocks: 20,
            cache_eviction_interval_seconds: 5,
            memory_monitoring_interval_seconds: 1,
            memory_pressure_threshold_bytes: 1024 * 1024, // 1MB
            enable_leak_detection: true,
        }
    }
    ...
    async fn test_l1_cache_basic_operations() {
        ...
        let cache = L1MemoryCache::<String, String>::new(config);

        // 测试设置和获取
        assert!(cache.set("key1".to_string(), "value1".to_string(), 10));
    ...
    async fn test_l1_cache_ttl_expiration() {
        ...
        config.l1_cache_ttl_seconds = 1; // 1秒过期
        let cache = L1MemoryCache::<String, String>::new(config);
    ...
    async fn test_l1_cache_eviction() {
        let mut config = create_test_config();
        config.l1_cache_max_entries = 3; // 最多3个条目
        let cache = L1MemoryCache::<String, String>::new(config);

        // 填满缓存
        cache.set("key1".to_string(), "value1".to_string(), 10);
        cache.set("key2".to_string(), "value2".to_string(), 10);
        cache.set("key3".to_string(), "value3".to_string(), 10);

        // 添加第4个条目，应该触发淘汰
        cache.set("key4".to_string(), "value4".to_string(), 10);

        let stats = cache.get_stats();
        assert!(stats.cache_evictions > 0 || stats.current_entries <= 3);
    }

    #[tokio::test]
    async fn test_object_pool_basic_operations() {
        let config = create_test_config();
        let pool = ObjectPool::new(|| Vec::<u8>::with_capacity(1024), config);
        ...
        // 归还对象
        let inner_obj = obj1.into_inner();
        pool.release(inner_obj);

        let stats = pool.get_stats();
        assert!(stats.pool_returns > 0);
    }
    ...
    #[tokio::test]
    async fn test_memory_pool_allocation_failure() {
        let mut config = create_test_config();
        config.memory_pool_max_blocks = 1; // 只允许1个块
        let pool = MemoryPool::new(config);

        // 分配第一个块
        let _block1 = pool.allocate().unwrap();
    ...
    async fn test_memory_manager_integration() {
        ...
        // 测试对象池操作
        let obj = manager.message_pool.acquire();
        assert!(obj.as_ref().is_some());

        // 测试内存池操作
        let block = manager.memory_pool.allocate();
        assert!(block.is_some());

        // 获取综合统计信息
        let stats = manager.get_comprehensive_stats();
        assert!(stats.cache_stats.cache_sets > 0);

        // 检查健康状态
        let health = manager.check_health();
        // 新创建的管理器应该是健康的
        assert!(health.is_healthy || health.issues.is_empty());
    ...
    #[tokio::test]
    async fn test_memory_monitor_pressure_detection() {
        let config = create_test_config();
    ...
    fn test_cache_stats_calculation() {
        ...
        // 执行一些操作来生成统计数据
        cache.set("key1".to_string(), "value1".to_string(), 10);
        cache.get(&"key1".to_string()); // 命中
        cache.get(&"nonexistent".to_string()); // 未命中

        let stats = cache.get_stats();
        assert_eq!(stats.cache_hits, 1);
        assert_eq!(stats.cache_misses, 1);
        assert_eq!(stats.hit_rate, 50.0); // 50% 命中率
    }
}
