...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager.create_table(
            Table::create()
                ...
                .col(
                    ColumnDef::new(Messages::MessageType).string_len(20).not_null().default("text")
                )
                // 消息状态：枚举值（sent, delivered, read, deleted, edited）
                .col(ColumnDef::new(Messages::Status).string_len(20).not_null().default("sent"))
                // 发送者用户ID：外键关联到users表
                ...
                // 消息优先级：0-9，默认为5
                .col(ColumnDef::new(Messages::Priority).integer().not_null().default(5))
                // 是否置顶消息：默认为false
                .col(ColumnDef::new(Messages::IsPinned).boolean().not_null().default(false))
                // 消息过期时间：可选，用于临时消息
                .col(ColumnDef::new(Messages::ExpiresAt).timestamp_with_time_zone().null())
                // 创建时间：自动设置为当前时间
                .col(
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        // 删除索引
        manager.drop_index(Index::drop().name("idx_messages_reply_to_id").to_owned()).await?;

        manager.drop_index(Index::drop().name("idx_messages_type_status").to_owned()).await?;

        manager.drop_index(Index::drop().name("idx_messages_sender_id").to_owned()).await?;

        manager.drop_index(
            Index::drop().name("idx_messages_chat_room_created_at").to_owned()
        ).await?;
